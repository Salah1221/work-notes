Imagine that you have tested your application in Debug mode and everything was working very well, then you publish your application and open it and... it doesn't behave like it did in Debug mode 😢. That bug could happen for several reasons, but for my case (in the prayer times app), the reason was that the AOT compilation, linker, or trimmer removing important JavaScript code used by C# that it thought is unused. I simply disabled all of them by adding the following code in the `.csproj` file:
...
>[!attention]
>Don't disabled those operations if they aren't causing any issues to your application
